...
export interface AccountIdentifierToBlobErr {
  kind: { InvalidAccountIdentifier: null } | { Other: null };
  message: [] | [string];
}
...
export type AccountIdentifier__1 =
  | { principal: Principal }
  | { blob: Array<number> }
  | { text: string };
...
export interface GetAccountIdentifierErr {
  kind: { InvalidToken: null } | { Other: null };
  message: [] | [string];
}
...
export interface GetBalanceErr {
  kind: { NotFound: null } | { InvalidToken: null } | { Other: null };
  message: [] | [string];
}
...
export interface GetDestinationAccountIdentifierErr {
  kind: { InvalidToken: null } | { InvalidInvoiceId: null } | { Other: null };
  message: [] | [string];
}
...
export interface Invoice {
  ...
  verify_invoice_avatar: ActorMethod<
    ...
    VerifyInvoiceResult
  >;
  verify_invoice_ticket: ActorMethod<[VerifyInvoiceArgs], VerifyInvoiceResult>;
}
...
export type TransferResult = { ok: TransferSuccess } | { err: TransferError };
export interface TransferSuccess {
  blockHeight: bigint;
}
export type UpdateCallsAggregatedData = Array<bigint>;
export interface VerifyInvoiceArgs {
  id: bigint;
}
...
export type VerifyInvoiceSuccess =
  | { Paid: { invoice: Invoice__1 } }
