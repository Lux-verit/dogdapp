...
import { idlFactory as idlFactoryGhost } from "./declarations/ghost/ghost.did";
import type Ghost from "./declarations/ghost/ghost.did.d";
...
const avatarID =
  process.env.NODE_ENV === "production"
    ? canisters.avatar.ic
    : canisters.avatar.local;
const accessoriesID =
  process.env.NODE_ENV === "production"
    ? canisters.accessories.ic
    : canisters.accessories.local;
const hubID =
  process.env.NODE_ENV === "production"
    ? canisters.hub.ic
    : canisters.hub.local;
const invoiceID =
  process.env.NODE_ENV === "production"
    ? canisters.invoice.ic
    : canisters.invoice.local;
const ledgerID =
  process.env.NODE_ENV === "production"
    ? "ryjl3-tyaaa-aaaaa-aaaba-cai"
    : canisters.ledger.local;
...
export function accessoriesActor(
  identity?: Identity,
): ActorSubclass<Accessories> {
...
export function ghostActor(identity?: Identity): ActorSubclass<Ghost> {
  return createActor("fjbi2-fyaaa-aaaan-qanjq-cai", idlFactoryGhost, {
    identity,
  });
}
