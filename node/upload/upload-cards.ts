...
console.log(
  "Deploying all cards to the accessory canister : " +
    accessoriesID +
    " on network : " +
    network,
);
...
function createAccessory(name: string, slot: string, recipe: string): Template {
  ...
  let file = readFileSync(
    `${__dirname}/../../assets/cards/accessories/${slot}/${small_name}/${small_name}-minified.svg`,
  );
  let content = file.toString();
  if (!content.includes(separator)) {
    throw new Error(
      `${name} is not in the correct format. Please check the svg file.`,
    );
  }
  let array = content.split(separator);
  if (array.length != 2) {
    throw new Error(
      "Something went wrong...array size should be two. Please check the svg file.",
    );
  }
...
function createMaterial(name: string): Template {
  let small_name = name.toLowerCase();
  let file = readFileSync(
    `${__dirname}/../../assets/cards/materials/${small_name}.svg`,
  );
  let array = [...file];
  return {
    Material: array,
