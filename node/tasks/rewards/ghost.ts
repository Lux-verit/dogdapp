...
async function distributeGhost(data: any) {
  ...
  for (let i = 0; i < data.length; i++) {
    const address = principalToAddress(
      Principal.fromText(data[i].Principal),
      null,
    );
    if (
      address ===
      "ebdb2741d815cc87b0f5fd71198b44705a0f740fa9d868b89c8c28b002d7e9c5"
    ) {
      skip = false;
    }
    if (skip) {
    ...
    const request: TransferRequest = {
      ...
      from: {
        principal: Principal.fromText(
          "ekhgh-6nthj-6np6u-yl3su-eri2j-tkaoo-533cq-w22q6-ofdrd-np2vv-bqe",
        ),
      },
      subaccount: [],
      nonce: [],
    ...
    const result = await Promise.all([
      await ghost.transfer(request),
      await accessories.record_token(
        address,
        amount,
        "IC GHOST Token",
        8,
        Principal.fromText("fjbi2-fyaaa-aaaan-qanjq-cai"),
      ),
    ]);
    console.log(result);
  }
