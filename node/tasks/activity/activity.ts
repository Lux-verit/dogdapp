...
async function collectAllEvents(): Promise<ExtendedEvent[]> {
  ...
  for (let collection of collections) {
    ...
    new_events = new_events.map((e) =>
      _extendEvent(e, collection[0].contractId),
    );
    new_events = new_events.filter((e) => _keepEvent(e));
    console.log(
      "Collected " + new_events.length + " events from " + collection[0].name,
    );
...
function _keepEvent(e: Event): boolean {
  return (
    e.operation == "burn" || e.operation == "mint" || e.operation == "sale"
  );
}
...
async function doJob() {
  ...
  let events = await collectEvents(
    BEGIN_TIME,
    Principal.fromText("qfevy-hqaaa-aaaaj-qanda-cai"),
  );
  let p = Principal.fromText(
    "yjyao-2xjnf-5yb7g-rdxfk-ktryb-ofb3h-fcwv5-fpruj-zoyrw-hba2r-eae",
  );
  let events_related = events.filter((e) => _isEventRelated(p, e));
...
function addToCollectionInvolved(
  collection: Collection,
  collection_involved: Collection[],
) {
