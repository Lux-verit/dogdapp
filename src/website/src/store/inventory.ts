...
import type {
  ...
  ItemInventory,
  Inventory,
} from "@canisters/accessories/accessories.did.d";
...
export function getMaterials(): Array<[string, TokenIdentifier]> {
  ...
  keys.forEach((key) => {
    if ("Material" in get(inventory)[key]) {
      materials.push([
        get(inventory)[key].Material.name.toLowerCase(),
        get(inventory)[key].Material.tokenIdentifier,
      ]);
    }
  });
  return materials;
...
export function getAccessories(
  inventory: Inventory,
): Array<[string, TokenIdentifier, boolean]> {
  var accessories = [];
  let keys = Object.keys(inventory);
  keys.forEach((key) => {
    if (inventory[key].Accessory) {
      accessories.push([
        inventory[key].Accessory.name.toLowerCase(),
        inventory[key].Accessory.tokenIdentifier,
        inventory[key].Accessory.equipped,
      ]);
    }
  });
  return accessories;
...
export function checkRecipe(recipe: Recipe): RecipeAnswer {
  ...
  for (let i = 0; i < recipe.length; i++) {
    ...
    const potential_material = materials.find(
      ([name, tokenIdentifier]) => name_material === name,
    );
    if (!potential_material) {
      ...
      materials = materials.filter(
        ([name, tokenIdentifier]) => tokenIdentifier !== potential_material[1],
      );
    }
  }
